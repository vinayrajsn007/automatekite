"""
NIFTY PE Auto Trader - Integrated Strategy
Implements: ADR-005-INTEGRATED-NIFTY-PE-AUTO-TRADER.md

Strategy Overview:
- Gets account balance to determine position sizing (40% risk factor)
- Takes NIFTY Options Expiry Date as user input
- Uses Options Scanner to select optimal PE options (premium ₹80-₹120)
...
class TraderConfig:
    """Configuration for Integrated NIFTY PE Auto Trader (ADR-005)"""
...
class IntegratedNiftyPETrader:
    """
    Integrated NIFTY PE Auto Trader (ADR-005)
    
    Combines:
    - ADR-003: Options Scanner (premium ₹80-₹120)
    - ADR-002: Double Confirmation PE Strategy
    ...
    def load_nifty_options(self):
        """Load NIFTY PE options for selected expiry"""
        if not self.expiry_date:
            logger.error("Expiry date not set")
            return []
        
        # Debug: Show available NIFTY PE expiry dates
        nifty_pe_expiries = set()
        for inst in self.nfo_instruments:
            if inst['name'] == 'NIFTY' and inst['instrument_type'] == 'PE':
                # Handle both datetime.date and datetime objects
                expiry = inst['expiry']
                if hasattr(expiry, 'date'):
                    expiry = expiry.date()
                nifty_pe_expiries.add(expiry)
        
        # Sort and show nearest expiries
        sorted_expiries = sorted(nifty_pe_expiries)
        logger.info(f"Target expiry: {self.expiry_date} (type: {type(self.expiry_date).__name__})")
        logger.info(f"Available NIFTY PE expiries (nearest 5): {sorted_expiries[:5]}")
        
        options = []
        for inst in self.nfo_instruments:
            if inst['name'] == 'NIFTY' and inst['instrument_type'] == 'PE':
                # Handle both datetime.date and datetime objects for comparison
                inst_expiry = inst['expiry']
                if hasattr(inst_expiry, 'date'):
                    inst_expiry = inst_expiry.date()
                
                if (inst_expiry == self.expiry_date and
                    self.config.STRIKE_MIN <= inst['strike'] <= self.config.STRIKE_MAX and
                    inst['strike'] % self.config.STRIKE_MULTIPLE == 0):
                    options.append(inst)
        
        logger.info(f"Found {len(options)} NIFTY PE options for {self.expiry_date}")
        return options
    ...
    def select_best_pe_option(self):
        """Select best PE option (ATM or closest to ₹100 premium)"""
        # Load options
        options = self.load_nifty_options()
        if not options:
            logger.warning("No PE options found for selected expiry")
            return None
        
        # Filter by premium range
        filtered = self.filter_by_premium_range(options)
        if not filtered:
            logger.warning("No PE options in premium range ₹80-₹120")
            return None
        ...
        for opt in filtered:
            if opt['strike'] == atm_strike:
                ...
                logger.info(f"Selected ATM PE: {opt['tradingsymbol']} @ ₹{opt['ltp']:.2f}")
                return opt
        ...
        logger.info(f"Selected PE: {best_option['tradingsymbol']} | Strike: {best_option['strike']} | Premium: ₹{best_option['ltp']:.2f}")
        return best_option
    ...
    def check_buy_conditions(self, df):
        """
        Check BUY conditions (ADR-002 Double Confirmation for PE)
    ...
    def display_status(self, nifty_spot, df_5min, df_2min, primary_signal, confirm_signal):
        ...
        print("\n" + "="*80)
        print(f"  NIFTY PE AUTO TRADER - {now.strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        if self.selected_option:
            print("\n  SELECTED PE OPTION (via Scanner)")
            print("  " + "-"*76)
            print(f"  Symbol: {self.selected_option['tradingsymbol']}")
            print(f"  Strike: {self.selected_option['strike']} (PE)")
...
Strategy:
• Option Type: PUT (PE) | Premium Range: ₹80-₹120 | Risk Factor: 40%
...
""")
        ...
        while True:
            try:
                ...
                # Select PE option via scanner
                if not self.selected_option or not self.position:
                    self.select_best_pe_option()
                
                if not self.selected_option:
                    logger.warning("No suitable PE option found. Waiting...")
                ...
                # Entry logic (no position)
                    ...
                    print("\n" + "="*80)
                    print("  >>> DOUBLE CONFIRMATION - EXECUTING BUY PE <<<")
                    print("="*80)
...
def main():
    """Main function to run Integrated NIFTY PE Auto Trader"""
    import argparse
    
    parser = argparse.ArgumentParser(description='NIFTY PE Auto Trader (ADR-005)')
    ...
    # Run trader
    trader = IntegratedNiftyPETrader(simulation=simulation)
