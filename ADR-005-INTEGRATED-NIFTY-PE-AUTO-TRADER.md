# ADR-005: Integrated NIFTY PE Auto Trader

**Status:** Accepted  
**Date:** January 2026  
**Author:** Trading System Team  
**Deciders:** Strategy Development Team  
**Integrates:** ADR-002 (Double Confirmation PE Strategy), ADR-003 (Options Scanner)
...
- Uses **ADR-003 Options Scanner** to select optimal PUT options (premium ₹80-₹120)
- Uses **ADR-002 Double Confirmation PE Strategy** to time entries and exits
- Automatically calculates quantity based on available balance (40% risk factor)
...
We will implement an **Integrated NIFTY PE Auto Trader** that combines the Options Scanner for instrument selection with the Double Confirmation Strategy for trade timing. The system operates in a **continuous loop**, executing multiple trades throughout the trading day until market close.
...
```
┌──────────────────────────────────────────────────────────────────────────────────┐
│                     INTEGRATED NIFTY PE AUTO TRADER                               │
│                    (ADR-003 + ADR-002 Combined System)                            │
├──────────────────────────────────────────────────────────────────────────────────┤
│                                                                                   │
│   ┌─────────────────────────────────────────────────────────────────────────┐   │
...
│   │   • Select Best PE Option (ATM or based on criteria)                   │   │
│   │   • Output: Selected PUT Option Instrument                              │   │
│   └─────────────────────────────────────────────────────────────────────────┘   │
│                                    │                                             │
│                                    ▼                                             │
│   ┌─────────────────────────────────────────────────────────────────────────┐   │
...
│                                    ▼                                             │
│   ┌─────────────────────────────────────────────────────────────────────────┐   │
│   │  PHASE 4: DOUBLE CONFIRMATION MONITORING (ADR-002)                      │   │
│   │  ──────────────────────────────────────────────────────────────────     │   │
...
| **Skip First Minutes** | 15 minutes | Skip 9:15-9:30 AM (volatile opening) |
...
│   ┌───────────────────────────────────────────────────────────────────────────┐ │
│   │                                                                            │ │
│   │   selected_option = scanner.get_best_pe_option()                          │ │
...
│   ┌─────────────────────────────────────────────────────────────────────────┐   │
│   │   Available PE Options:                                                  │   │
│   │   ─────────────────────                                                  │   │
│   │   | Strike | Premium | Distance from ATM |                              │   │
│   │   |--------|---------|-------------------|                              │   │
│   │   | 25600  | ₹115.25 | +100 (ITM)        |                              │   │
...
│   ┌─────────────────────────────────────────────────────────────────────────┐   │
│   │                                                                          │   │
│   │   Priority 1: ATM Strike PE (if premium in range)                       │   │
│   │   Priority 2: Nearest OTM PE (if ATM not in range)                      │   │
│   │   Priority 3: Nearest ITM PE (if no OTM in range)                       │   │
│   │                                                                          │   │
...
│   ┌─────────────────────────────────────────────────────────────────────────┐   │
│   │   {                                                                      │   │
│   │     "tradingsymbol": "NIFTY26JAN25500PE",                               │   │
...
## Double Confirmation Entry Logic (From ADR-002)
...
│   │       if quantity > 0:                                                  │   │
│   │           buy_option(                                                    │   │
│   │               tradingsymbol = selected_pe_option['tradingsymbol'],      │   │
│   │               quantity = quantity,                                       │   │
│   │               order_type = "MARKET"                                     │   │
│   │           )                                                              │   │
│   │       else:                                                              │   │
...
│   ═══════════════════════════════════════════════════════════════════════════    │
│                                                                                   │
│   9:15 AM    9:30 AM                                                3:30 PM      │
│      │          │                                                       │        │
│      │  SKIP    │  ┌──────┐ ┌──────┐ ┌──────┐ ┌──────┐             │        │
│      │  ZONE    │  │Trade │ │Trade │ │Trade │ │Trade │    ...      │        │
│      │  15 min  │  │  #1  │ │  #2  │ │  #3  │ │  #4  │             │        │
│      │ (volatile)│  └──────┘ └──────┘ └──────┘ └──────┘             │        │
│      │          │                                                       │        │
│      ├──────────┼───────────────────────────────────────────────┬───────┤        │
│      │   SKIP   │         ACTIVE TRADING ZONE                    │ STOP  │        │
│      │  OPENING │        (New trades allowed)                    │ ZONE  │        │
│      │  15 min  │                                                │15 min │        │
│      └──────────┴───────────────────────────────────────────────┴───────┘        │
│                                                                                   │
│   TRADE CYCLE FLOW:                                                              │
...
│   SAFETY FEATURES:                                                               │
│   ────────────────                                                               │
│   • **Skip first 15 minutes (9:15-9:30 AM) - volatile opening period**          │
...
│  T+4s:   Initial Scanner Run - Select PE Option                                  │
...
│  │  │   2. **Skip if Opening Period (9:15-9:30 AM) - Wait for 9:30 AM**   │  │  │
│  │  │   3. Refresh Account Balance                                         │  │  │
│  │  │   4. Scan & Select New PE Option                                    │  │  │
...
```
════════════════════════════════════════════════════════════════════════════════════
  NIFTY PE AUTO TRADER - 2026-01-23 10:35:15
...
════════════════════════════════════════════════════════════════════════════════════
  ...
  DOUBLE CONFIRMATION STATUS (ADR-002)
  ────────────────────────────────────────────────────────────────────────────────
...
════════════════════════════════════════════════════════════════════════════════════
  ...
  TRADE DETAILS:
  ──────────────────────────────────────────────────────────────────────────────────
  #1: NIFTY26JAN25500PE | Entry ₹95.50 → Exit ₹98.25 | P&L: +₹1,237.50 | ema_low_falling
  #2: NIFTY26JAN25600PE | Entry ₹112.00 → Exit ₹108.50 | P&L: -₹1,575.00 | strong_bearish
  #3: NIFTY26JAN25500PE | Entry ₹94.00 → Exit ₹99.00 | P&L: +₹2,250.00 | ema_low_falling
  #4: NIFTY26JAN25400PE | Entry ₹82.00 → Exit ₹85.00 | P&L: +₹1,337.50 | market_close
════════════════════════════════════════════════════════════════════════════════════
  Goodbye!
```
...
│   │
│   └── select_best_pe_option()
│       └── Select ATM or nearest suitable PE option
│
├── QUANTITY CALCULATOR
...
│
├── DOUBLE CONFIRMATION METHODS (ADR-002)
...
### Interactive Mode
```bash
python integrated_nifty_pe_trader.py
```

### Programmatic Mode
```python
from integrated_nifty_pe_trader import IntegratedNiftyPETrader

# Initialize trader
trader = IntegratedNiftyPETrader()
...
- `ADR-002-NIFTY-DOUBLE-CONFIRMATION-OPTION-PE-BUY-STRATEGY BUY.md` - Double confirmation PE strategy
- `ADR-003-NIFTY-OPTIONS-SCANNER.md` - Options scanner strategy
- `integrated_nifty_pe_trader.py` - Main implementation
...
|---------|------|--------|---------|
| 1.0 | Jan 2026 | Team | Initial Integrated ADR combining ADR-002 and ADR-003 |
